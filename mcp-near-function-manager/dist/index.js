...
function parseContractCode(base64Code) {
    const functions = [];
    try {
    ...
    try {
        switch (name) {
            case "list_functions": {
                ...
                const code = codeResult?.code_base64 || "";
            ...
            case "estimate_gas": {
                ...
                const gasUsed = result?.result?.gas_burnt || "unknown";
                return {
                    content: [{
                            type: "text",
                            text: JSON.stringify({
            ...
            case "compare_functions": {
                const accountId2 = args?.account_id_2;
                ...
                const functions1 = parseContractCode(code1?.code_base64 || "");
                const functions2 = parseContractCode(code2?.code_base64 || "");
