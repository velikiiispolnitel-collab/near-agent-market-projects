...
describe('decodeAction', () => {
  ...
  test('decodes CreateAccount', () => {
    const result = decodeAction({ type: 'CreateAccount', params: {} });
  ...
  test('handles unknown action type', () => {
    const result = decodeAction({ type: 'UnknownAction', params: {} });
...
describe('formatNearAmount', () => {
  test('formats whole NEAR', () => {
    expect(formatNearAmount('1000000000000000000000000')).toBe('1.0000 NEAR');
    expect(formatNearAmount('5000000000000000000000000')).toBe('5.0000 NEAR');
  });

  test('formats thousands', () => {
    const result = formatNearAmount('1000000000000000000000000000');
    expect(result).toContain('K NEAR');
  });

  test('formats small amounts', () => {
    const result = formatNearAmount('1000000000000000000000');
    expect(result).toContain('mNEAR');
  });

  test('formats raw yocto', () => {
    const result = formatNearAmount('1');
...
describe('formatPermission', () => {
  test('formats FullAccess', () => {
    expect(formatPermission('FullAccess')).toBe('Full Access');
  });
  ...
  test('handles unknown', () => {
    expect(formatPermission({})).toBe('Unknown');
  });
});
...
describe('ABI registry', () => {
  test('lookups known method', () => {
    expect(lookupMethod('v2.ref-finance.near', 'swap')).toBe('Swap tokens on Ref Finance');
    expect(lookupMethod('wrap.near', 'near_deposit')).toBe('Wrap NEAR to wNEAR');
  });

  test('returns null for unknown', () => {
    expect(lookupMethod('unknown.near', 'method')).toBeNull();
  });

  test('registers custom ABI', () => {
    registerABI('custom.near', {
      methods: {
        myMethod: { description: 'My custom method' },
      },
    });
    expect(lookupMethod('custom.near', 'myMethod')).toBe('My custom method');
  });
});
